...
class AdonisjsTestCommand(sublime_plugin.WindowCommand):
    def get_paths(self):
        file_name = self.window.active_view().file_name()
        app_path = self.find_app_path(file_name)
...
class runAdonisjsTestCommand(AdonisjsTestCommand):

    def run(self, *args, **kwargs):
        file_name, app_path, active_view, directory = self.get_paths()

        self.run_in_terminal('cd ' + app_path + ' && npm run test -- --filter=' + file_name)

class RunAllAdonisjsTestsCommand(AdonisjsTestCommand):

    def run(self, *args, **kwargs):
        file_name, app_path, active_view, directory = self.get_paths()

        self.run_in_terminal('cd ' + app_path + ' && npm run test')
